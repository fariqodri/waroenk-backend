...
export class createdAtSellerBankNullable1599814882157 implements MigrationInterface {
    name = 'createdAtSellerBankNullable1599814882157'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("ALTER TABLE `posts` CHANGE `image` `image` varchar(255) NULL DEFAULT NULL");
        await queryRunner.query("ALTER TABLE `seller_bank` CHANGE `created_at` `created_at` datetime NULL");
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("ALTER TABLE `seller_bank` CHANGE `created_at` `created_at` datetime NOT NULL");
